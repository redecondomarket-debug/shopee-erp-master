...
import { DollarSign, TrendingUp, TrendingDown, Download, FileText, Search, X } from 'lucide-react'
...
export default function FinanceiroPage() {
  ...
  async function loadData() {
    setLoading(true)
    const { data } = await supabase.from('financeiro').select('*').order('data', { ascending: false }).limit(500)
    setItems(data || [])
    setLoading(false)
  }
  ...
  const exportExcel = () => exportToExcel(filtered.map(i => ({
  ...
  const exportPdf = () => exportToPDF('Relatório Financeiro', [
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {(search || lojaFilter || dateFrom || dateTo) && (
          <button onClick={() => { setSearch(''); setLojaFilter(''); setDateFrom(''); setDateTo('') }} className="btn-secondary">
