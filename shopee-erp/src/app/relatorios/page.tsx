...
function PizzaCustosFixos({ itens }: { itens: { label: string; v: number; cor: string }[] }) {
  ...
  const slices = itens.map((item, idx) => {
    ...
    const s = ang, e = ang + pct * 2 * Math.PI
    ang = e
    ...
    <div style={{ padding: '20px 24px', boxSizing: 'border-box' as any }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
        ...
        <div style={S.card}>
          ...
          <div style={{ height: 12 }} />
          <LinhaResultado label="(=) Margem de Contribuição" v={margContrib} cor={margContrib >= 0 ? '#a78bfa' : '#ef4444'} totalRec={totalRecOp} vAnt={margContribAnt} />
          <div style={{ height: 8 }} />
